...
`microsoft/vscode` を fork した **独自エディタ「Para Code」**。目的は、拡張機能APIの境界に縛られず、
AIエージェント（Claude Code CLI / Codex CLI 等）と密結合した開発体験をネイティブに作り込むこと。

- 配布は自前チャネル限定（VSIX拡張ではなく fork 版エディタ本体を配る）。macOS / Apple Silicon が第一ターゲット、Win/Linux も視野。
...
| 付随 | Para Code設定セクション + ウィンドウ透明度 | ✅ 完了（ブラー保留） | `src/vs/paradis/contrib/windowTransparency/`（`6a206d7ad47`） |
...
| 機能1拡張 | Rename / Set Color（Superset同一12色）/ Reveal in Finder / Copy Path | ✅ 完了（実機検証済み） | `paradisWorkspacesView.ts` |
| 機能1拡張 | git worktree の入れ子ツリー表示 + 自動追加/削除（Para Code設定 `autoImportWorktrees`/`autoRemoveMissingWorktrees`）+ worktree への切替 | ✅ 完了（実機検証済み） | `paradisWorktreeService.ts` |
| 機能1拡張 | エージェント実行状態インジケータ（hooks→`/agent-hook`→スピナー/赤脈動/緑ドット、`Para Code: Copy Agent Hooks Setup`） | ✅ 完了（実機検証済み） | `paradisAgentStatus.contribution.ts` + shared process |
| 付随 | 通知サウンド + Notifications設定UI（Superset同等: 着信音11種/カスタム音源/YouTube取込+波形エディタ/Aivis読み上げ/ユーザー辞書/使用量） | ✅ 完了（UI実機確認済み。音・Aivis実再生とYouTube取込は人手での確認推奨） | `src/vs/paradis/contrib/notifications/` |
| 付随 | CPU/RAMモニタ（タイトルバー左インジケータ + 内訳パネル、Superset移植） | ✅ 完了（実機検証済み） | `src/vs/paradis/contrib/resourceMonitor/` |
| 付随 | Explorer をデフォルトで右サイドバー（SCMの下）へ | ✅ 完了（実機検証済み） | `explorerViewlet.ts` PARA-PATCH |
| 付随 | 素の起動時に必ずデフォルトワークスペースを開く（Superset風） | ✅ 完了（新規プロファイルで実機検証済み） | `paradisDefaultWorkspace.ts` + `windowsMainService.ts` PARA-PATCH |
...
内蔵ブラウザのページを、ターミナルペインで動く外部エージェントCLI（Claude Code / Codex）から読み書きさせる。
...
- **UI（Phase C）**: `paradisBindingDialog.ts`（自前モーダル、`--vscode-*` トークン、mock準拠）。ブラウザツールバーボタン、
  コマンド `Para Code: Open Agent Browser Binding`、ターミナルグリッドセルのインジケータ、ステータスバー項目から開く。
  バインドはトークンさえあれば常に可能（エージェント検出やMCP接続状態でブロックしない）。
- セットアップは `Para Code: Copy MCP Setup Command`（Claude Code / Codex / browser-use を選んでスニペットをコピー）。
...
- **使い方**: `Para Code: Initialize Multi-Repo Workspace` → サイドバーの Workspaces ビュー（+ボタン / `Para Code: Add Repository...`）で
