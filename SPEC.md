...
| 機能1拡張 | git worktree の入れ子ツリー表示 + 自動追加/削除（Paradis設定 `autoImportWorktrees`/`autoRemoveMissingWorktrees`）+ worktree への切替 | ✅ 完了（実機検証済み） | `paradisWorktreeService.ts` |
| 機能1拡張 | エージェント実行状態インジケータ（hooks→`/agent-hook`→スピナー/赤脈動/緑ドット、`Paradis: Copy Agent Hooks Setup`） | ✅ 完了（実機検証済み） | `paradisAgentStatus.contribution.ts` + shared process |
