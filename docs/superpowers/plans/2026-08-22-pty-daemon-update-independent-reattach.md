...
- **旧い常駐に、書かれているのに繋がれていない方針が2つある（2026-08-22 に発見、未対処）。**
  `paradisJudgeForeignDaemon`（別ビルドの常駐が1本も抱えていなければ黙って終了させる＝更新の
  たびに古い常駐が積み上がるのを防ぐ）と `paradisProbeDaemonRecord`（台帳を読んだ直後の判断）は、
  **本番の呼び出し元が無く、テストからしか呼ばれていない**。`paradisDaemonRecordFile` は参照ゼロ。
  意図はコメントに残っているので消さずに置いてあるが、**テストがあるぶん「効いている」と読めて
  しまう**状態。繋ぐと「アプリが常駐を自動で終了させる」という振る舞いの変更になるので、
  判断を仰いでから。なお新しい常駐は protocol の版で鍵を切るため、ビルドごとに積み上がる問題
  自体が起きない。
