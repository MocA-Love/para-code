...
## 更新履歴（アプリ内changelog）の運用（2026-07-03整備・2026-07-28にモバイル版追加、厳守）

PC版・モバイルアプリ版のそれぞれに、ユーザーが読む更新履歴がある。**どちらも「実装した作業の中で書く」のがルール**で、後からまとめて書き起こさない。

### PC版（Para Code デスクトップ）
...
アプリを更新した後の初回起動で「アップデートのお知らせ」シートが出る。閉じたあとは設定 →「更新履歴」からいつでも読み返せる（`app/mobile/src/components/updateSheet.tsx` と `app/mobile/app/changelog.tsx`）。

- 実体は **`app/mobile/src/changelog.ts` の `MOBILE_CHANGELOG`**。起動時のお知らせと設定の一覧が同じ配列を読むので、書く場所は常にここ1箇所
- **`app/mobile/` 配下をユーザー向けに変更したら、その作業の中で先頭バージョンの `items` に1件追記する**（コミットに含める）。内部整備のみなら書かない
- 1項目は `{ icon, title, body?, tone? }`。`title` は「何ができるようになったか / 何が直ったか」を短い一文で、`body` は補足1〜2文。**PC版と同じくファイル名・クラス名は書かない**。`icon` は Ionicons の名前
- その版の目玉がある場合だけ `headline` を書く（シートの大見出しになる）。**内部整備だけの版は `items` を空にする**（空の版はお知らせを出さない）
