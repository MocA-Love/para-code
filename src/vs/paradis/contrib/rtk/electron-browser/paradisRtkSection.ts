...
// RTK 節約ダッシュボードの描画本体。rtk CLI の集計結果(shared process 経由)を
// KPI カード・日別の節約推移・コマンド別内訳・直近のコマンドとして描画する。
// チャートは素の SVG DOM で構築し、workbench テーマのCSS変数+固定のアクセント色を使う。
//
// ホスト(タブ / 統合ダイアログ)に依存しないよう、EditorPane からは切り離してある。
// 幅はホストから layout(width) で渡してもらう — ダイアログのように開いた直後は
// 実測できない場所でもチャートを組み立てられるようにするため。
...
/** body の左右パディング(CSS の .paradis-rtk-body と合わせる)。幅を実測できない時の逆算に使う。 */
const BODY_HORIZONTAL_PADDING = 40;

/** 幅をまだ一度も知らされていない時に仮定する外枠の幅。 */
const DEFAULT_HOST_WIDTH = 900;
...
export class ParadisRtkSection extends Disposable implements IParadisUsageSection {
	...
	private readonly presetButtons: { key: ParadisRtkPresetKey; button: HTMLButtonElement }[] = [];
	private readonly granularityButtons: { granularity: ParadisRtkGranularity; button: HTMLButtonElement }[] = [];
	...
	private hostWidth = DEFAULT_HOST_WIDTH;
	...
	/** まだ一度も取得していなければ取得する(ホストが表示を始めた時に呼ぶ)。 */
	ensureLoaded(): void {
	...
	layout(width: number): void {
		...
		const bodyWidth = this.currentBodyWidth();
		if (this.data && Math.abs(bodyWidth - this.lastRenderedWidth) > 4) {
	...
	/**
	 * チャートの組み立てに使う body の内寸。DOM に載っていれば実測を優先し、
	 * まだ測れない場所(開いた直後のダイアログなど)ではホストから渡された幅から逆算する。
	 */
	private currentBodyWidth(): number {
		const measured = this.body.clientWidth;
		return measured > 0 ? measured : Math.max(0, this.hostWidth - BODY_HORIZONTAL_PADDING);
	}
	...
	private renderBody(): void {
		...
		dom.clearNode(this.body);
		this.hideTooltip();
		this.lastRenderedWidth = this.currentBodyWidth();
	...
	private renderSavedChart(card: HTMLElement, buckets: IBucket[], granularity: ParadisRtkGranularity): void {
		...
		const accent = this.accent;
		const width = Math.max(320, card.clientWidth > 0 ? card.clientWidth - 34 : this.currentBodyWidth() - 74);
		const height = 235;
