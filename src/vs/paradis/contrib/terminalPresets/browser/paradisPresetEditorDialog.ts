...
// allow-any-unicode-next-line
const STR_COMMANDS = localize('paradis.presetEditor.commands', "コマンド（1行に1つ）");
// allow-any-unicode-next-line
const STR_ICON = localize('paradis.presetEditor.icon', "アイコン（codicon 名。例: rocket, play, server-process, debug-start）");
// allow-any-unicode-next-line
const STR_CWD = localize('paradis.presetEditor.cwd', "作業ディレクトリ（任意。相対パスはリポジトリルート基準）");
// allow-any-unicode-next-line
const STR_LAUNCH_MODE = localize('paradis.presetEditor.launchMode', "起動モード");
// allow-any-unicode-next-line
const STR_MODE_CURRENT = localize('paradis.presetEditor.mode.current', "アクティブなターミナルで実行（&& 連結）");
// allow-any-unicode-next-line
const STR_MODE_NEW = localize('paradis.presetEditor.mode.new', "新しいターミナルで実行（&& 連結）");
// allow-any-unicode-next-line
const STR_MODE_EACH = localize('paradis.presetEditor.mode.each', "コマンドごとに新しいターミナル");
// allow-any-unicode-next-line
const STR_MODE_SPLIT = localize('paradis.presetEditor.mode.split', "分割して並べる（コマンドごと）");
// allow-any-unicode-next-line
...
const LAUNCH_MODE_LABELS: readonly { mode: ParadisPresetLaunchMode; label: string }[] = [
	{ mode: 'new-terminal', label: STR_MODE_NEW },
	{ mode: 'current-terminal', label: STR_MODE_CURRENT },
	{ mode: 'new-terminal-each', label: STR_MODE_EACH },
	{ mode: 'split', label: STR_MODE_SPLIT },
];

export function openParadisPresetEditorDialog(accessor: ServicesAccessor): void {
	const dialog = new ParadisPresetEditorDialog(
...
class ParadisPresetEditorDialog extends Disposable {
	...
	private _renderList(): void {
		...
		for (const preset of presets) {
			...
			dom.append(main, $('.ppe-row-detail')).textContent = preset.description || preset.commands.join(' && ');
	...
	private _renderEdit(editing: IParadisResolvedPreset | undefined): void {
		...
		const commandsInput = dom.append(field(STR_COMMANDS), $('textarea.ppe-input.ppe-commands')) as HTMLTextAreaElement;
		commandsInput.rows = 4;
		commandsInput.spellcheck = false;
		commandsInput.value = editing ? editing.commands.join('\n') : '';
		...
		updateIconPreview();
		this._viewStore.add(dom.addDisposableListener(iconInput, 'input', updateIconPreview));
		...
		const modeSelect = dom.append(field(STR_LAUNCH_MODE), $('select.ppe-input.ppe-select')) as HTMLSelectElement;
		for (const { mode, label } of LAUNCH_MODE_LABELS) {
			const option = dom.append(modeSelect, $('option')) as HTMLOptionElement;
			option.value = mode;
			option.textContent = label;
		}
		modeSelect.value = editing?.launchMode ?? 'new-terminal';
		...
		const checkbox = (label: string, checked: boolean): HTMLInputElement => {
			...
			this._viewStore.add(dom.addDisposableListener(labelEl, 'click', () => { input.checked = !input.checked; }));
			return input;
		};
		...
		this._viewStore.add(dom.addDisposableListener(saveBtn, 'click', async () => {
			...
			const definition: IParadisPresetDefinition = {
				...
				launchMode: modeSelect.value as ParadisPresetLaunchMode,
