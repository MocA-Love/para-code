...
import { IParadisMcpSetupResult, PARADIS_MCP_PORT_FILE_ENV_VAR, PARADIS_PANE_TOKEN_ENV_VAR, ParadisMcpCli } from '../common/paradisAgentBrowser.js';
...
function assertBoundedRegularConfig(stat: IConfigFileStat): void {
	if (stat.isSymbolicLink() || !stat.isFile()) {
		throw new Error('Codex configuration is not a regular file');
	}
	if (!Number.isSafeInteger(stat.size) || stat.size < 0 || stat.size > MAX_CODEX_CONFIG_BYTES) {
		throw new Error('Codex configuration exceeds the safe read limit');
	}
}
...
async function readConfigSnapshot(
	...
	assertBoundedRegularConfig(statBefore);
	...
	try {
		...
		assertBoundedRegularConfig(openedStat);
		...
		assertBoundedRegularConfig(openedStatAfter);
		assertBoundedRegularConfig(stat);
