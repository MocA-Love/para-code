...
export function paradisGetNotifyScriptContentPs1(): string {
	return [
		'# Para Code agent notification hook schema v3 (PowerShell)',
		'# Auto-generated by Para Code (paradisAgentHooksSetup.ts); regenerated on startup.',
		'# Forwards Claude Code / Codex hook events to the Para Code shared process',
		'# (POST /agent-hook). Outside Para Code terminals stdin is drained and discarded.',
		...
		'# pid ($PID = this PowerShell process) lets the shared process resolve the',
		'# emitting agent process via the ancestor chain (nested-agent guard, schema v3).',
		'try {',
		`  $hookUri = "http://127.0.0.1:$port/agent-hook?pane=$($env:${PARADIS_PANE_TOKEN_ENV_VAR})&event=$eventName&pid=$PID"`,
