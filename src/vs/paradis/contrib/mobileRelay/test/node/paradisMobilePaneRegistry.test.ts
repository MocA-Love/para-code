...
suite('ParadisMobilePaneRegistry', () => {
	...
	test('旧Rendererの遅延disposeで新Rendererの対応表を消さない', () => {
		...
		registry.syncWindow(1, 'old', 1, [{ terminalId: 1, token: 'pane' }]);
		registry.syncWindow(1, 'current', 2, [{ terminalId: 8, token: 'pane' }]);
	...
	test('生存中Rendererの空同期後に作成されたペインを同じsessionで登録できる', () => {
		const registry = new ParadisMobilePaneRegistry();
		assert.strictEqual(registry.syncWindow(1, 'current', 2, []), true);
		assert.strictEqual(registry.syncWindow(1, 'current', 2, [{ terminalId: 8, token: 'pane' }]), true);
	...
	test('current sessionのremoveだけが対応表を削除する', () => {
		...
		assert.strictEqual(registry.syncWindow(1, 'current', 2, [{ terminalId: 9, token: 'late' }]), false);
	});

	test('同じtokenを複数の生存ウィンドウが登録した場合は解決しない', () => {
		const registry = new ParadisMobilePaneRegistry();
		registry.syncWindow(1, 'one', 1, [{ terminalId: 1, token: 'pane' }]);
		registry.syncWindow(2, 'two', 2, [{ terminalId: 1, token: 'pane' }]);
	...
	test('新世代の後から届いた旧世代syncとdisposeを拒否する', () => {
		const registry = new ParadisMobilePaneRegistry();
		registry.syncWindow(1, 'new', 2, [{ terminalId: 2, token: 'new-pane' }]);

		assert.strictEqual(registry.syncWindow(1, 'old', 1, [{ terminalId: 1, token: 'old-pane' }]), false);
		assert.strictEqual(registry.removeWindow(1, 'old', 1), false);
