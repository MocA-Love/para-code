...
import {
	PARADIS_SPREADSHEET_SERIALIZER_REGISTRATIONS,
...
import { PARADIS_SPREADSHEET_VIEWER_REGISTRATION, registerParadisOfficeViewerSerializers } from '../browser/paradisOfficeConfiguration.js';
...
registerParadisOfficeViewerSerializers(Registry.as<IEditorFactoryRegistry>(EditorExtensions.EditorFactory), PARADIS_SPREADSHEET_SERIALIZER_REGISTRATIONS);

// 差分の旧版は git: スキーム(git拡張のreadonly FSプロバイダ)で渡ってくるため、canSupportResource で許可する必要がある。
const SUPPORTED_SCHEMES = new Set<string>(PARADIS_SPREADSHEET_VIEWER_REGISTRATION.schemes);
...
class ParadisSpreadsheetViewerResolverContribution implements IWorkbenchContribution {
	...
	constructor(
		...
		for (const ext of PARADIS_SPREADSHEET_VIEWER_REGISTRATION.extensions) {
			editorResolverService.registerEditor(
				paradisGlobForExtension(ext),
				{
					id: PARADIS_SPREADSHEET_VIEWER_REGISTRATION.editorId,
