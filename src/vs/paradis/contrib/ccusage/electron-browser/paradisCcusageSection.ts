...
// ccusage ダッシュボードの描画本体。ccusage CLI の集計結果(shared process 経由)を
...
//
// タブ(EditorPane)と統合ダイアログのどちらからも同じ中身を出せるよう、ここはホストに
// 依存しない。幅と可視状態はホストから layout()/setVisible() で渡してもらう。
...
/**
 * ccusage ダッシュボードの中身一式。EditorPane タブと統合ダイアログの両方が
 * この1クラスを包む。
 */
export class ParadisCcusageSection extends Disposable implements IParadisUsageSection {
	...
	constructor(
		...
		this.customRangeRow = dom.append(this.element, $('.paradis-ccusage-custom-range'));
		...
		this.tooltip = dom.append(this.element, $('.paradis-ccusage-tooltip'));
	...
	/** まだ一度も取れていなければ初回取得を始める。取得中/取得済みなら何もしない。 */
	ensureLoaded(): void {
	...
	layout(width: number): void {
		const changed = Math.abs(width - this.lastRenderedWidth) > 4;
	...
	private renderBody(): void {
		...
		dom.clearNode(this.body);
		this.hideTooltip();
		this.lastRenderedWidth = this.availableWidth;
