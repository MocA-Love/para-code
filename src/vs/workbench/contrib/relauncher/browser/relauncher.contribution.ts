...
// PARA-PATCH: import the ext-host restart suppression flag for Para Code multi-repo workspace switching (see paradisWorkspaceSwitch.ts)
...
export class WorkspaceChangeExtHostRelauncher extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this.extensionHostRestarter = this._register(new RunOnceScheduler(async () => {
			...
			if (environmentService.isSessionsWindow || isParadisManagedWorkspaceWindow()) { // PARA-PATCH: also skip the restart for Para Code multi-repo workspace switching
