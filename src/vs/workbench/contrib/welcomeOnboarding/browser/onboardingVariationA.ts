...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to {0}", product.nameLong)); // PARA-PATCH: brand the onboarding aria-label with the product name
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to {0}", product.nameLong); // PARA-PATCH: brand the onboarding hero title with the product name
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all {0} commands.", product.nameShort), // PARA-PATCH: brand the command-palette tip with the product name
		);
	}
