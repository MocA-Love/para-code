...
export class BrowserViewInspector extends Disposable {
	...
	getElementHandle(id: string, frame: Electron.WebFrameMain): IElementHandle | undefined {
		...
		return {
			...
			// PARA-PATCH: forward the element's outerHTML for the "Copy Element" context menu item.
			// Upstream 1.132 replaced the plain pass-through of the frame handle with this wrapper,
			// which forwards members one by one, so the fork member has to be forwarded explicitly.
			getOuterHTML: () => handle.getOuterHTML(),
