...
| `resources/paradis/extensions/ms-azuretools.vscode-containers-2.4.106.vsix` | 新規追加（Para Codeパッチ版のContainer Tools拡張、MIT）。upstream `microsoft/vscode-containers` v2.4.5 をベースに、Containersビューをワークスペーススコープに絞る機能（設定 `containers.containers.scopeToWorkspace`、既定オン。composeの `com.docker.compose.project.working_dir` ラベルが現在のワークスペースフォルダと無関係なコンテナを隠す。composeでないコンテナは常に表示）と、フォルダ入れ替え時の即時リフレッシュを追加。WSLのUNCパス（`\\wsl$\...` / `\\wsl.localhost\...`）はLinuxパスへ変換して照合するため、Windowsローカルウィンドウで `\\wsl$` フォルダを開きWSL内composeを使う構成でも誤って隠れない。バージョンはforkを示すためpatch+100系の `2.4.106`。forkソースはローカル `~/github/vscode-containers-paradis`（ブランチ `paradis-workspace-scope`。変更点: 新規 `src/tree/containers/paradisWorkspaceScope.ts`、`ContainersTreeItem.ts` へのPARA-PATCH 2箇所、`package.json`/`package.nls.json` の設定追加）。再ビルドは `npm ci && npm run build:esbuild && npm run package`。`installGivenVersion: true` によりpinnedとなり自動更新では上書きされない（既存のギャラリー版が入っていてもVSIXインストールが置き換える）。upstream拡張の新版へ追従する際はタグにrebaseして同手順で再生成 | スペース切り替え後もContainersビューに他スペースのcomposeコンテナが表示され続ける問題の解消（SCMの `paradisScmRepoScope` に相当する拡張側の対応） |
...
- **デプロイ先**: CROUTECHアカウント（account_id: `979dbe0328e903a34bb6291b06cca0da`、上記の更新配信基盤と同一アカウント）。ユーザーが明示的にこのアカウントを指定して選定
...
- Version ID: `c63e07c0-c10a-49fa-91d6-6bb11863c8ae`
