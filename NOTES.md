...
| `package.json` / `package-lock.json` | `dependencies` に `exceljs@^4.4.0` を追加（`npm install exceljs@4.4.0 --save --ignore-scripts`、54パッケージ追加） | Excelビューア/差分機能の xlsx パース。exceljs は Buffer/stream 依存で renderer(sandbox)では動かないため shared process（`src/vs/paradis/contrib/fileViewers/node/`）でのみ使用。eslint の node層import許可リスト（`eslint.config.js` の hasNode `allow`）にも `'exceljs'` を PARA-PATCH で追加済み |
