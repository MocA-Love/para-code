...
| `build/lib/stylelint/vscode-known-variables.json` | `others` に `--paradis-transparency-opacity` / `--paradis-titlebar-bg` / `--paradis-statusbar-bg` / `--paradis-workspace-color` / `--paradis-pr-color` / `--paradis-agent-live-row-height` / `--paradis-status-breathe` / `--paradis-status-pulse` を追加。**注意: 初回追加時に `others` 配列全体（約70行）をアルファベット順に再ソートしたため、diffは純増分より大幅に広い（±70行超）**。upstream取り込みでこのファイルがコンフリクトした場合、upstream側の配列を丸ごと採用し、上記paradis 8変数だけ再挿入するのが最も安全 | ウィンドウ透過機能・Workspacesビュー色バー・PR状態チップ・エージェント一覧のタイル高さのカスタムCSS変数（`--paradis-agent-live-row-height` は paracode-93 で `--paradis-agent-live-min-row` から改名）。`--paradis-status-breathe` / `--paradis-status-pulse` はWorkspacesビューの状態ドットの明滅をリスト全体で同位相にするための時計（`@property` 宣言つき）。hygiene の stylelint (Unknown variable) を通すため |
...
| `product.json` | `builtInExtensions` に `mobile-canvas-vscode@0.1.16`（Marketplace上の実体は `redth.mobile-canvas`、MIT）を追加。**`sha256` は空文字にし、代わりに upstream 由来の `platformSpecific` フィールドへ6ターゲット（darwin/linux/win32 × x64/arm64）分のvsix checksumを列挙する**。metadataのUUIDは Visual Studio Marketplace の extensionquery から取得 | iOSシミュレータ/Androidエミュレータをライブ表示・操作する Mobile Canvas を標準同梱するため。**`name` を `redth.mobile-canvas` にしてはいけない**: `platformSpecific` 指定時のGitHub Releaseアセット名は `build/lib/extensionTarget.ts` の `getPlatformSpecificAssetName()` が `<name>-<target>.vsix` で組み立てるため、配布実体の `mobile-canvas-vscode-darwin-arm64.vsix` に合わせて `name` を `mobile-canvas-vscode` にする必要がある（`name` は `.build/extensions/<name>/` のフォルダ名にしか使われず、拡張のIDは同梱 `package.json` の `publisher`+`name` から決まるので実害はない）。`platformSpecific` 指定の拡張は `extensionsGallery`（fork は Open VSX）へフォールバックせず常にGitHubから取る仕様のため、Open VSX未公開でも同梱できる。**プラットフォーム別vsix（12〜14MB）はネイティブランタイムを内包しており初回起動時のダウンロードが発生しない**が、`mobile-canvas-vscode.vsix`（universal、77MB）や `-thin.vsix` は実行時にランタイムを取りに行くため同梱には使わない。**注意: `platformSpecific` を宣言した拡張はアセットの無いターゲットでビルドすると `resolvePlatformSpecificAsset()` が throw してビルド全体が落ちる**。Para Codeのリリース対象（darwin x64/arm64・win32 x64/arm64・linux x64）は6ターゲットに全て含まれるが、alpine や linux-armhf でローカルビルドする場合は該当アセットが無く失敗する。版を上げる際は `gh release download <tag> --repo Redth/mobile-canvas-ghcp --pattern "mobile-canvas-vscode-*.vsix"` して `shasum -a 256` で6件を実測し直す（リリース同梱の `SHA256SUMS` はランタイム `.gz` のみでvsixを含まない） |
