...
- **構成**: ランチャーは `codex.cmd`（cmd用）/`codex.ps1`（PowerShell用）の薄い入口 + `paradisCodexPaneLauncher.cjs`（本体）。JSは `PARA_CODE_CODEX_LAUNCHER_NODE`（Para Code自身のexe）を `ELECTRON_RUN_AS_NODE=1` で実行する。`.cmd`は`call`を使わずexeを直接呼ぶ（`call ... %*`は埋め込み引用符・`&`で引数が壊れることを実測済み）。実ポートは `userData\pcx\<token>.endpoint.json` に書き、shared process（`paradisCodexLiveClient`）がそれを読んで `ws://127.0.0.1:<port>` へBearer付きで直接続する
- **実Codexの解決**: PATHからランチャー自身のディレクトリを除外した上で、`codex.exe`直接 → npmインストール（`node_modules/@openai/codex/bin/codex.js`を自Nodeで実行、またはvendored `codex.exe`を探索） → 拡張子なし`codex`（非Windowsのdev/test用）の順
